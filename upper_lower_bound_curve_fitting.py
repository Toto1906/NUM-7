...
def fitted_curves_plot(N_values_vec, error_values_vec, mem_values_vec, time_values_vec):
    ...
    print(f"Power law fit mem: mem ≈ {mem_fit[0]:.2e} · N^(-{mem_fit[1]:.3f}), R² = {r2_mem:.4f}")
    print(f"Power law fit time: time ≈ {time_fit[0]:.2e} · N^(-{time_fit[1]:.3f}), R² = {r2_time:.4f}")
    ...
    plt.plot(N_smooth, time_smooth, 'r-',
             label=f"Power law fit time: time ≈ {time_fit[0]:.2e} · N^(-{time_fit[1]:.3f}), R² = {r2_time:.4f}")
    plt.xlabel('N')
    plt.ylabel('Time')
